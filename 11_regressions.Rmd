...
Adding independent variables to a regression model often happens in a stepwise approach, i.e. we do not add all variables at once. Instead we might add variables that are most important (based on prior research) first, run the regression and examine the results. After that we might add more variables that could possibly add value to our regression model and run the regression again. We then have two models which we can compare and see whether we improve our model. Most frequently though, we consider three tpyes of independent variables:

1.  main effects: These are the independent variables that the researchers expects to best predict the independent variable

2.  control variables: These are independent variables which might affect the independent variable in some way and we want to make sure it is not influential (or too influential)

3.  interaction variables: These are variables that moderate the strength of the relationship between independent and depend variables.

Hierarchical regressions are most frequently used for the latter two options.

`CONTINUE FROM HERE`

Hierarchical regressions are a special type of multiple regression were we predetermine the order in which we add variables to our model. There are many other ways of perform multiple, for example starting with all variables and removing those that are not significant, which @cohen2014applied calls a 'tear-down' approach (p. 158). As the title indicates, we take a less exploratory approach to our analysis, because we define the hierarchy, i.e. the order, in which we enter variables. In the Social Sciences this is very often the preferred method., which is why I cover it in greater detail.
...
1.  Create a model `m1` which only contains the dependent variable `new_cases` and a control variable `country`.
2.  Inspect the results of this model and note down the performance measures of this model.
3.  Create another model `m2` and include the control variable `country` and all other independent variables of interest, i.e. `masks`, `movements`, `gatherings`, `schools`, and `businesses`.
4.  Inspect the results of this model and note down the performance measures of this model.
5.  Compare the models and see whether they are significantly different from each other. We can use `anova()` to perform this step.

Let's put these steps into action and start with formulating our first model. I choose `country` as a control variable, because we have sufficient evidence that clustered data could be a reason for the high autocorrelation of residuals we found in Chapter \@ref(multicollinearity-the-dilemma-of-highly-correlated-independent-variables). For all computations in this chapter we use the original dataset, i.e. `covid_uk_ger`, because we changed the model and therefore would have to revisit outliers from scratch, which we shall skip at this point. We also have to remove observations with missing data to allow comparisons of models in a statistical way. We begin my first selecting our variables of interest and then remove missing data.
...
hr_data2 <- hr_data %>%
  mutate(country = as.numeric(case_when(country == "United Kingdom" ~ "1",
                                        country == "Germany" ~ "0")))
...
There are many more combinations of models we could test. For example, instead of specifying two models (`m1`, `m2`) we could also add independent variables one at a time and compare it to a baseline model (`m0`) which does contain the control variable, which means we end up with four models to compare:

::: {#four-hierarchical-regressions-nested}
m0 \<- lm(formula = new_cases \~ schools + businesses, data = hr_data)
...
While hierarchical regressions help us make decisions on whether we should include or exclude variables, moderation models imply that we expect a certain interaction between an independent variable and a so-called moderator. A moderator also enters the equation on the right-hand side and therefore constitutes an independent variable.

Computationally an interaction is defined by the multiplication of an independent variable with the moderating variable. For example, instead of assuming that country is a control variable, we might think that the measures for `business` and `schools` were significantly differently implemented in each of the countries. Therefore, the scores of `business` and `schools` vary depending on the country we collected it from.

When performing a moderation regression, we assume that the relationship between independent variables and dependent variables differ, for example the country we look at. If we visualise the idea of moderation we would plot the data against each other for each country and fit two (instead of one) regression line by defining the colours of each group with `col = country`.

```{r Plotting moderation effects, echo=TRUE}
...
# Use the package 'patchwork' to stitch them together
# library(patchwork) # If you have not done so yet
p1 + p2
```

There are two interesting insights we can gain from these plots:
...
2.  The UK reports more frequently higher numbers of `new_cases` even if measures taken are high, which results in a steeper regression line. We find this particularly with `schools`, where `Germany` has barely any new cases when the measures were high, but the UK still reports very high numbers of new cases when tight measures were taken.

Given the difference in the slope of the regression line, we have to assume that the $\beta$s for `Germany` are different from the ones for the `United Kingdom`. Does, the relationship between `new_cases` and `business`/`schools` can be partially explained by `country`. Therefore, we could include country as a moderator and introduce the interaction of these variables with each other into our model.

```{r Moderated regression, echo=TRUE}

# COUNTRY REQUIRES DUMMY CODING TO WORK PROPERLY
...
The moderation effect is quite strong. Compared to our baseline model `m0`, which only contains `businesses` and `schools` as predictor, our new model `m4` considerably outperforms it with an $adjusted\ R^2 = 0.307$. If we consider our previous model with country as a control variable, we only explained 25.5% of the variance in `new_cases`. This quite an improvement.

However, if we take a look at the regression coefficients, we find that once we add this moderation effect, `schools` is not a significant predictor anymore. Comparing the $\beta$ values with each other, `country` is the strongest predictor, followed by the moderator `businesses * country`. Similar to before, with a desire to achieve a parsimonious model, we can remove the non-significant predictor.

```{r Model after moderation effects accounted for, echo=TRUE}
m5 <- lm(formula = new_cases ~
           businesses +
           businesses * country, # moderator 2
         data = hr_data)

parameters::compare_parameters(m4, m5, standardize = "refit") %>%
  as_tibble() %>%
  select(Parameter, Coefficient.m4, p.m4, Coefficient.m5, p.m5) %>% 
  mutate(across(where(is.double), round, 3))
