...
covid_bin <- covid %>%
  na.omit() %>%
  select(-date_reported, -date_updated, -first_vaccine_date, masks:movements) %>%
  correlationfunnel::binarize(n_bins = 5)

covid_bin_corr <- covid_bin %>% correlationfunnel::correlate(target = new_cases__575_Inf)

covid_bin_corr %>% correlationfunnel::plot_correlation_funnel()
```
...
m1 <- lm(new_cases ~ masks + travel + businesses + schools, data = covid_uk)
broom::tidy(m1) %>% mutate(across(where(is.numeric), round, 2))
