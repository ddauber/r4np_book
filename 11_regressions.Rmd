...
Since we also plotted the observations that affect the variable `travel`, we can notice that there are seemingly less outliers for this predictor.

#### Reviewing the outliers {#reviewing-the-outliers}

After all this hard work and what turned out to be a very lengthy chapter, we finally arrive at the point to check which observations we might wish to remove or handle in some shape or form. First, we want to know which observations are affected. Therefore we need to review our reference data `outliers`. What we want to see are all observations which were identified by one or more diagnostic tool as an outlier. There is two ways to achieve this. We can use what we have learned so far and simply `filter()` each column for the value `TRUE`. This is the hard way of doing it and if you have many more columns this will take a little while. The easy (and clever) way of filtering across multiple columns can be achieved with the function `across()`. This function works similar to the `select()` function, but it takes two arguments which can be expressed as `across(.cols, .fns)`. The argument `.cols` refers to the columns we want to filter (or perform any other computation), and `.fns` allows us to define the function that should be used across all columns defined in `.cols`. In our case, we want to select all columns but `rowname` and return all values that `TRUE`. Therefore, we can perform all these steps with one line of code

```{r Reviewing the outliers, echo=TRUE}
outliers %>% filter(if_any(everything(), ~ .x == "TRUE"))
```

The result is a `tibble` which tells us there were in total 18 outliers detected.
