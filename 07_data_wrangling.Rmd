...
When using `glimpse()` we find the recognised data type for each column, i.e. each variable, in `<...>`, for example `<chr>`. We will return to data types in Chapter \@ref(change-data-types). Lastly, we get samples of the data included. This output is much more helpful.
...
From Chapter \@ref(coding-etiquette), you will remember that being consistent in writing your code and naming your objects is essential. The same applies, of course, to variable names. *R* will not break using the existing names, but it will save you a lot of frustration if we take a minute to clean the names and make them more consistent. Since we will use column names in almost every line of code we write, it is best to standardise them and easier to read.
...
If you need to convert all `<chr>` columns you can use `mutate_if(is.character, as.factor)` instead. This function will look at each column and if it is a `character` type variable, it will convert it into a `factor`. However, use this function only if you are certain that all `character` columns need converting. If this is the case, it can be a huge time-saver.

## Handling factors {#handling-factors}

Factors are an essential way to classify observations in our data in different ways. In terms of data wrangling, there are usually two steps we take to prepare them for analysis:

-   Recording factors, and
...
Besides changing factor levels, it is also common that we want to reconsider the arrangement of these levels. To inspect the order of factor levels, we can use the function `fct_unique()` from the `forcats` package and provide a factor variable, e.g. `gender`, in the `wvs` dataset.
...
As the plot demonstrates, we managed to obtain a dataset which is now free of missing values and we can being our next steps of data wrangling or analysis.
...
An example of exploratory factor analysis is presented in Chapter ???. Since the `gep` data is based on an established measurement tool, we perform a CFA. To perform a CFA, we use the popular `lavaan` ([*La*]{.ul}*tent [Va]{.ul}riable [An]{.ul}alysis*) package. The steps of running a CFA in *R* include:
...
The `broom` package is useful to clean output from all kinds of models, such as a CFA model. It allows us to convert the raw output into a `tibble`, which we can further manipulate using the functions we already know. However, it is not required to use `tidy()` to inspect the results. This can be done out of pure convenience. If we want to know where `names` and `x` came from we have to inspect the output from `broom::tidy(fit_indices)`.
...
-   The `rmsea` (*Root Mean Square Error of Approximation*) appears slightly higher than desirable, which usually is `0.6`. [@hu-bentler-1999cutoff], and
