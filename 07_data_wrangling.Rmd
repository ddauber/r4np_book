...
## Handling dates, times and durations {#handling-dates-times-and-durations}
...
### Converting dates and times for analysis {#converting-dates-and-times-for-analysis}
...
It would be nice if there was a standardised method of recording dates and times and, in fact, there is one: [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html "ISO 8601: A standard for reporting and recording dates and times"). But unfortunately, not every software, let alone scholar, necessarily adheres to this standard. Luckily there is a dedicated *R* package that solely deals with these issues called `lubridate`, which is also part of the `tidyverse`. It helps reformat data into a coherent format and takes care of other essential aspects, such as '*time zones, leap days, daylight savings times, and other time related quirks*' [@lubridate]. Here, we will primarily focus on the most common issues, but I highly recommend looking at the ['lubridate' website](https://lubridate.tidyverse.org/ "Website of the package 'lubridate'"){target="_blank"} to see what else the package has to offer.
...
```{r example-date-conversation, echo=TRUE, message=FALSE}
library(lubridate)
...
```{r quest-time-data-clean, echo=TRUE}
quest_time_clean <-
  quest_time %>%
...
### Computing durations with dates, times and date-time variables {#computation-dates-times}
...
-   [mice](https://cran.r-project.org/web/packages/Amelia/index.html "mice"){target="blank"} (Uses multivariate imputation via chained equations)
...
#### Reversing items with `mutate()` {#reversing-items-mutate}
...
```{r reverse-coding-mutate, echo=TRUE}
df_social_r <-
  df_social %>%
  mutate(q2_r = 7 - q2)
...
#### Reversing items with `recode()` {#reversing-items-recode}
...
```{r reverse-coding-recode-01, echo=TRUE}
df_social_r <-
  df_social %>%
...
```{r reverse-coding-recode-02, echo=TRUE}
df_social_r <-
  df_social %>%
...
#### Reversing items with the `psych` package {#reversing-items-psych}
...
```{r reverse-coding-psych, echo=TRUE}
...
#### A final remark about reversing item scores {#reversing-items-final-remark}
...
```{r recreate-social-r, include=FALSE}
df_social_r <-
  df_social %>%
  mutate(q2_r = 7 - q2)
```
