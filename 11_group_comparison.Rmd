...
As we can see, the distribution of `age` across both `gender` groups is fairly similar and likely not different between groups. Of course, we could also statistically test this using a group comparison test before performing the main group comparison. However, we first have to understand how we can perform them. Unfortunately, there is yet no unified way of computing all these statistics in a more `tidyverse` approach. The package `rstatix` has several meaningful ways of conducting group comparisons using pipe friendly functions. However, they do not play well along with the package `effectsize`. Thus, it very much depends on what you prefer. I opted to use as much basic R functions as possible for the computation. Still, the lack of a uniform notation can be a bit frustrating. Thus, be aware of how functions are constructed in this section.
...
If we find a significant difference between two groups, we have to ask ourselves: How big is the difference? The answer to this is provided by the effect size. The interpretation of what the effect size is, follows largely the explanations in Chapter \@ref(correlations), where we look at the strength of the correlation of two variables. However, different effect size measures imply that we have to use different benchmarks. To help us a bit with the interpretation we can use the `correlation` package and their set of `interpret_...()` functions (see also [Indices of Effect Size](https://easystats.github.io/effectsize/articles/interpret.html "Indices of Effect Size"){target="blank"}). Sometimes, there are more than one way of computing the effect size. For example for the Mann-Whitney test we can choose between the classic Wilcoxon R, computed as $\frac{Z}{\sqrt{n}}$ or the rank-biserial correlation coefficient. In practice, you have to be explicit about how you computed the effect size. The differences between the two measures are often marginal and a matter of taste (or what your reviewers prefer).
