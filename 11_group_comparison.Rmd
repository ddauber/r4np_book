...
As we can see, the distribution of `age` across both `gender` groups is fairly similar and likely not different between groups. Of course, we could also statistically test this using a group comparison test before performing the main group comparison. However, we first have to understand how we can perform them. The package `statsExpressions` has several meaningful ways of conducting group comparisons using pipe friendly functions which can be found on the package's [website](https://indrajeetpatil.github.io/statsExpressions/index.html "website of statsExpressions"){target="blank"}. These will be used throughout this chapter, because they are in line with a `tidyverse` approach to data analysis and also provide effect sizes for our tests. However, I provided the underlying functions related to each test in Chapter \@ref() opted to use as much basic R functions as possible for the computation in this chapter, because (1) it helps you better understand which functions are used in the `statsExpressions` package, and (2) they integrate well with so-called 'post-hoc' tests, which we have to perform for multiple group comparisons. Nevertheless, I highly recommend to take a look at the `statsExpressions` package, which is not only hugely popular, but is helpful in standardising how we write our tests in R and also switching between different methods very quickly without rewriting an entire set of code.
...
test <- comp %>%
  statsExpressions::two_sample_test(x = gender,
                                    y = freedom_of_choice,
                                    type = "nonparametric",
                                    paired = FALSE,
                                    var.equal = TRUE)
