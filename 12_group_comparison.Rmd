...
If each month was measured with a different group of participants we would compare the differences between each month. However, since we look at paired data, we need to consider the differences in pairs of measures. In our study the following combinations are possible:
...
The output from `anova_test()` also provides information labelled as `Sphericity Corrections`. These we need to consider if the sphericity assumption is violated, i.e. Mauchly's Test if Sphericity is significant. We commonly use two methods to correct for the degrees of freedom in our analysis: Greenhouse-Geisser correction [@greenhouse1959methods] and Huynh-Field correction [@huynh1976estimation]. @field2013discovering explains that if the Greenhouse-Geisser estimate (i.e. `GGe`) is greater than `0.74`, it is advisable to use the Huynh-Field correction instead. For each correction, the `anova_test()` provides corrected p-values. In our example, we do not have to worry about corrections.

#### Visualising and computing multiple paired group comparisons {#visualising-computing-multiple-paired-group-comparisons}

So far we blindly assumed that our data is parametric. If our assumptions for parametric tests are violated, we can draw on a non-parametric equivalent called `Friedman Test`.
...
Instead of using a fictional example as in the previous chapter, let's draw on some real observations in `wvs_waves`. This dataset contains similar data to `wvs`, but multiple measures for the same country. For example, we might interested to know whether `satisfaction` across different countries changed over the years.

Let's start by visualising the groups and measures using a `geom_boxplot()`.
...
In total we plotted 21 boxplots comparing three countries across seven waves. We can tell that `satisfaction` with life has not changed much for people in `Japan`. They are more or less equally satisfied throughout the years. However, we notice some increases in `satisfaction` for `Argentina` and `Mexico`. There appear to be also quite some outliers, which we ignore for the sake of simplicity.

Next, we would have to check for our assumptions to conduct a parametric test. Since we have covered this in Chapter \@ref(sources-of-bias) we only consider the Mauchly's Test of Sphericity. Our data is currently b

```{r Repeated measures ANOVA, echo=TRUE}
# Compute repeated measures ANOVA and
# Mauchly's Test of Sphericity

wvs_waves %>% rstatix::anova_test(dv = satisfaction,
                                  wid = country,
                                  within = wave)
...
Similar to paired group comparisons it is possible that contingency tables show paired data. For example, we could be interested in knowing whether certain training we delivered improved participants skills (or at least their personal perception).
