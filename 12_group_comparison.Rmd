...
The mosaic plot shows that more participants indicate `post-training` that they feel more confident in communicating with others, i.e. they replied with '`yes`, I feel more confident'. There were only very few participants who indicated that they have not become more confident.

Let's get more specific and compute the corresponding contingency table.

```{r Contigency table paired comparison 2by2, echo=TRUE}
...
# Wrong table - this is more for chi-squared tests
ic_training %>%
...
With these insights we can refine our interpretation and further state that after the intercultural training, 98% of participants. This is quite a substantial improvement, given that only 43% of participants felt already confident. Still, we want to be as precise as possible and therefore should conduct the corresponding significance test. For paired 2x2 contingency tables we have to use McNemar's Test, using the function `mcnemar.test()`. To determine the effect size we have to compute Cohen's g and use the function `cohens_g()` from the `effectsize` package to compute it.

```{r Contigency tables paired computation, echo=TRUE}
#McNemar's test
mcnemar.test(ic_training$test, ic_training$communication2)

# Effect size
effectsize::cohens_g(ic_training$test, ic_training$communication2)
```
