...
anova_ev <- aov(satisfaction ~ country, data = mcomp)
summary(anova_ev)

effectsize::eta_squared(anova_ev)
...
If each month measured a different group of participants we would compare the differences between each month regarding homogeneity. However, since we look at paired data, we need to consider the differences in pairs of measures. In our study the following combinations are possible:
...
Let's add the differences between measures for each participant based on these pairs and compute the variance of these values.
...
We can tell that the differences across groups is fairly small when comparing the values of `m1_m4_var` and `m4_m8_var`. However, the value for `m1_m8_var` appears bigger. So, how can we be certain whether the assumption of sphericity is violated or not? Similar to many of the other assumptions we covered, there is a significance test which provides the answer to this question. For multiple paired groups, we use the Mauchly's Test of Sphericity. If the test is significant, the variances across groups are not equal. In other words, a significant Mauchly test implies a violation of sphericity. The `rstatix` package includes Mauchly's Test of Sphericity in its `anova_test()`. Thus, we get both with one function. However, our data in this example is not tidy. We first need to convert it into a tidy dataset.
...
Instead of using a fictional example as we did in the previous chapter, let's draw on some real observations using the dataset `wvs_waves`. This dataset contains similar data to `wvs`, but offers multiple measures indicates in the variable `wave`. For example, we might be interested to know whether `satisfaction` changed over the years.
...
In total we plotted 7 boxplots for each of the waves of data collection. We can tell that `satisfaction` with life has improved slightly, especially from wave 3 (`w3`) to wave 4 (`w4`). There appear to be also quite some outliers, which we ignore for the sake of simplicity.

Next, we would have to check the assumptions for parametric tests. Since we have covered this in Chapter \@ref(sources-of-bias) we only consider the Mauchly's Test of Sphericity by running `anova_test()`.
...
We first take a look at the sphericity assumption which we did not violate, i.e. $p > 0.05$. Thus, using the ANOVA test is appropriate. Next we take a look at the ANOVA test results and find that the differences are significant as well, i.e. $p < 0.05$. Still, the effect size `ges` is small.

From our plot, we know that some of the waves are fairly similar to each other and we need to determine which pair of waves is truly significantly different from each other. Therefore, we are in need of a post-hoc test which allows us to perform pairwise comparisons. By now, this should sound oddly familiar (see also Table \@ref(tab:post-hoc-tests).
...
In view of the above, we can confirm that `satisfaction` with life has changed positively for our participants, but the change is very small (statistically).

To finish this chapter, I want to share the non-parametric version of this test, i.e. the Friedman test, i.e. `friedman_test()` from `rstatix`.
...
# Effect size
wvs_waves %>% effectsize::kendalls_w(satisfaction ~ wave | id)
```
...
A more common and compact way to show such dependencies between categorical variables is a contingency table. To convert our current table into a contingency table we need to map the levels of `satisfaction_bin` as rows (i.e. `satisfied` and `unsatisfied`), and for `gender` we want each level represented as a column (i.e. `male` and `female`). This can be achieved with the function `pivot_wider()`. It turns our 'long' data frame into a 'wide' one. Let's take a look at the output first, which is much easier to understand.
...
Similar to previous group comparisons, we can distinguish between paired and unpaired groups. I will cover each comparison in turn and also allude to ways of plotting two or more categories in a `ggplot()`.

### Unpaired groups of categorical variables {#unpaired-groups-categorical-variables}

In the introduction to this chapter we covered a classic example of an unpaired comparison of two groups (`male` and `female`) regarding another categorical variable, i.e. `satisfaction_bin`. The tables we produced offered detailed insights into the distribution of these categories. However, it is always helpful to have a visual representation. To plot two categorical variables (i.e. factors) in `ggplot2` can be achieved in many different ways. Three commonly used options are shown in Figure \@ref(fig:three-plots-satisfaction-gender).
...
A more elegant and compact way of visualising frequencies across two or more categories are mosaic plots. These visualise relative frequencies for both variables in one plot. Consider the following mosaic plot created with `ggmosaic` and the function `geom_mosaic()`.

```{r Visualising a contingency table with mosaics, echo=TRUE, fig.cap="A mosaic plot which visualises the relationship of two categorical variables", label="mosaic-plot"}
library(ggmosaic)
wvs_nona %>%
  ggplot() +
...
Throughout the remaining chapters I will use the mosaic plot to illustrate distributions. A main difference to regular bar plots is the function `product()`, which allows us to define different variables of interest instead of using `x` and `y` in the `aes()` of `ggplot()`. It is this function which enables a mosaic visualisation. Apart from that, the same ggplot syntax applies.
...
To cut a long story short, we only need to be worried about 2x2 contingency tables where the values in some (or all) cells is lower than 10. In those cases we need to rely on the Fisher's Exact Test. In all other cases we can rely on the Pearson Chi-squared Test to do our bidding. The function `infer::chisq_test()` is based on the function `chisq.test()`, which automatically applies the required Yate's Continuity Correction if necessary.
...
Both tests reveal that the relationship between our variables is not significant and the effect sizes are very small. This result is in line with our visualisation shown in Figure \@ref(fig:mosaic-plot), because everything looks very symmetrical.
...
In a next step, we want to compute the contingency table. However, there is one aspect we need to account for: The responses are paired and therefore we need a contingency table of paired responses. this requires some modification of our data. Let's start with a classic contingency table as we did for unpaired data.

```{r Contigency table paired comparison 2by2 before pairing , echo=TRUE}

# ic_training %>% infer::chisq_test(test ~ communication2,
#                                correct = TRUE)
# 
# (phi <- effectsize::phi(ic_training$test, ic_training$communication2))
...
This contingency table reflects the mosaic plot we created, but it does not show paired answers. If we sum the frequencies in each cell we receive a score of `96`, which equals the number of observations in `ic_training`. However, since we have two observations per person (i.e. paired responses), we only have 48 participants. Thus, this frequency table treats each the pre and post-training group as being independent from each other.

The current dataset includes two rows for each participant and if we want to create a contingency table with paired scores, we need to first convert our data in such a way that each participant is reflected by one row only, i.e. we need to make our data frame wider with `pivot_wider()`.

```{r Convert dataset to enable paired contingency table, echo=TRUE}
# Creating data frame reflecting paired responses
paired_data <- ic_training %>%
  pivot_wider(id_cols = name,
              names_from = test,
              values_from = communication2)
```

In simple terms, we converted the column `communication2` into two columns based on the factor levels of `test`, i.e. `pre-training` and `post-training`. If we now use these two columns to create a contingency table and mosaic plot, we get an understanding of how the paired distributions between `pre-training` and `post-training` look like.
...
-   However, for the other 29 participants (60%) the training helped them to change from `no` to `yes`. This is an excellent result.

-   We notice that there was not a single participant who scores `no` after the training was delivered, which is also a great achievement by those hosting the training session.

Lastly, we need to perform a statistical test to confirm our suspicion that the training had a significant impact on participants. For paired 2x2 contingency tables we have to use McNemar's Test, using the function `mcnemar.test()`. To determine the effect size we have to compute Cohen's g and use the function `cohens_g()` from the `effectsize` package to compute it. Be aware that we have to use the `paired_data` as our data frame and not `ic_training`.
...
Thus, our effect size is very large and we can truly claim that the intercultural training had a significant impact on the participants. Well, there is only one flaw in our analysis. An eager eye will have noticed that one of our cells was empty, i.e. the top-right cell in the contingency table. The conditions to run such a test are similar to the Chi-squared Test. Thus, using the McNemar test is not quite appropriate in our case. Instead we need to use the 'exact McNemar test' which compares the results against a binomial distribution and not a chi-squared one . More important than remembering the name or the distribution is to understand that for smaller samples, the exact test produces more accurate results. However, we have to draw on a different package to compute it, i.e. `exact2x2` and its function `mcnemar.exact()`.

```{r Compute exact McNemar test}
library(exact2x2)
mcnemar.exact(paired_data$pre_training, paired_data$post_training)
```

Even after using a more robust computation for our data, we find that the results are still significant. Thus, there is no doubt that the intercultural training helped participants to improve.

There many other combinations of contingency tables. The McNemar test can be extended to a 3x3 or higher matrix, but the rows and columns always have to be the same length. If you want to compare multiple categorical variables it would be necessary to look into a loglinear analysis. While this book does not cover this technique, an excellent starting point for this is provided by @field2013discovering (p. 732ff).

### A final remark about comparing groups based on categorical data

Admittedly, I hardly ever find studies in the field of Social Sciences which work with primarily categorical data. Political Science might be an exception, because working, for example, with polling data usually implies working with categorical data. A reason why you likely find yourself in the situation to work with such data and analytical techniques is measurement accuracy. If you look at the `ic_training`, the variable `communication2` was artificially created to turn `numeric` data into a `factor`. This is usually not good practice, because you are losing measurement accuracy and it tends to exaggerate differences between groups. Consider the following plots:
...
The more we aggregate the data into fewer categories, the more likely we increase differences between groups. The plot `Category with 3 levels` shows that most participants fall into the category `medium`. However, reducing the categories to two, most participants are classified as `yes`. Thus, some participants which were classified as `medium` are now in the `no` category. However, in reality, several participants will still have improved in confidence, but are not considered together with those who have not improved. This is a major problem, because now it seems there are more people who are confident about communicating with people from different cultural backgrounds.

In short, I recommend to only use the techniques outline in this chapter if your data is truly categorical in nature. Thus, when designing your data collection tool, you might also wish to abstain from measuring quantitative variables as categories, for example `age`, which is very popular among my own students. While it might be sometimes more appropriate to offer categories, it is always good to use multiple categories and not just two. Every additional level in your factor will increase measurement accuracy and provides access to more advance analytical techniques. Remember: it is always possible to convert `numeric` data into a `factor`, but not the other way around.

## Reviewing group comparisons {#reviewing-group-comparisons}

In Social Sciences we almost always find ourselves comparing individuals against each other or with themselves over time. As such, understanding how to compare groups of participants is an essential analytical skill. The techniques outline in this chapter merely provide a solid starting point and likely cover 80% of datasets you will collect.
