...
```{r popular-genre-barplot-extras-formatted, echo=TRUE}
ggplot(imdb_top_250, aes(x = genre_01)) +
  geom_bar() +
...
```{r popular-genre-barplot-step-by-step, echo=TRUE}
...
## Frequencies and relative frequencies: Counting categorical variables {#frequency}

One of the most fundamental descriptive statistics includes counting. We often we find ourselves having data from different groups of participants, and we might be interested in knowing how many are in each group. For example, we might be interested to know which director produced most movies listed in the IMDb Top 250. Each director is a category in the variable `director`. We already used the required function to compute such a frequency when we looked at movie genres (see Chapter \@ref(plotting-in-r-with-ggplot2)), i.e. `count()`.
...
```{r frequency-filtered, echo=TRUE}
imdb_top_250 %>%
  count(director) %>%
  filter(n >= 6)
```
...
Besides *absolute frequencies*, i.e. the actual count of occurrences of a category, we sometimes wish to compute *relative frequencies* to make it easier to compare categories more easily. *Relative frequencies* are more commonly known as *percentages*. Consider the following example, which reflects a study where participants were asked to rate a new movie after watching it.
...
```{r inspect-movie-ratings, echo=TRUE}
...
It seems we have a very even distribution of `male` and `female` participants. Thus, our analysis would not be affected by having more participants from one or the other gender. However, if we found that most participants were `female` (e.g. 80%) then any further computation we perform with this dataset would reflect the opinion of female viewers more than male ones. This is important to know because it limits the representativeness of our results. The insights gained from primarily female participants in this study would be not helpful when trying to understand male audiences.
...
```{r density-vs-normal-bw-settings, echo=TRUE, results='hide'}
# bw = 18
n_plot +
  geom_density(bw = 18) +
  ggtitle("bw = 18")
